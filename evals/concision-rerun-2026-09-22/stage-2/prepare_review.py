...
for a in ['none','old','candidate']:
 for rep in [1,2]:
  for d in json.loads((R/f'{a}-{rep}-output.json').read_text())['outputs']:
   ...
   if 'punctuation only' in c['request'].lower() or 'punctuation-only' in c['request'].lower():checks['punctuation_only']=re.findall(r'\w+',c['source'])==re.findall(r'\w+',d['text'])
...
(R/'experiment.json').write_text(json.dumps({'primary_category':'generate','repetitions':2,'cases':cases,'outputs':rows},indent=2))
